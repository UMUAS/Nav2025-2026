#Calculates optical flow of live color stream from an OAK-D.
...
with dai.Pipeline() as pipe:
    ...
    rgbQ = cam.requestOutput((640, 400)).createOutputQueue()
    ...
    while pipe.isRunning():
        ...
        cv.imshow('RGB', cv.threshold(frame, minval_val, 255, cv.THRESH_BINARY)[1])
        cv.imshow('Value', val)
