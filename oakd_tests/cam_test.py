...
def getSystemInformation(info):
    m = 1024 * 1024 # MiB
    t = info.chipTemperature
    return [f"Ddr used / total - {info.ddrMemoryUsage.used / m:.2f} / {info.ddrMemoryUsage.total / m:.2f} MiB",
...
with dai.Device(*dai_device_args) as device:
    ...
    sys_info_q = device.getOutputQueue("sys_log", 4, False)
    ...
    while True:
        ...
        for c in streams:
            ...
            if pkt is not None:
                ...
                elif c.startswith('cam') and cam_type_thermal[cam_skt]:
                ...
                if len(frame.shape) != 3 or frame.shape[2] != 3: frame = cv2.cvtColor(frame, cv2.COLOR_GRAY2BGR)

                if sys_info:
                    cv2.putText(frame, '*', (10, 20), cv2.FONT_HERSHEY_TRIPLEX, 0.7, (0,255,0))
                    most_recent_sys_info = getSystemInformation(sys_info)

                if most_recent_sys_info:
                    for i in range(len(most_recent_sys_info)): cv2.putText(frame, most_recent_sys_info[i], (10, 40 + 20*i), cv2.FONT_HERSHEY_TRIPLEX, 0.7, (0, 255, 0))
                cv2.imshow(c, frame)
